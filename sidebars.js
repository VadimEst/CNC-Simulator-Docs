module.exports = {
  userSidebar: {
    'Project': ['user/overview', 'user/installation', 'user/functionality', 'user/commands','user/examples'],
  },
  devSidebar: {
    'Project': ['dev/overview', 'dev/contribute', 'dev/getting-started'],
    'General Setup': ['dev/scoping', 'dev/architecture', 'dev/ui-concept', 'dev/libraries'],
    'cnc.service': ['dev/code/logger', 'dev/code/programparser', 'dev/code/settings', 'dev/code/settingsparser', 'dev/code/terminalreader'],
    'cnc.service.exceptions': ['dev/code/milloutofboundsexception','dev/code/invalidcommandexception','dev/code/invalidprogramfileexception'],
    'cnc.tools': ['dev/code/tool'],
    'cnc.ui': ['dev/code/settingspopover', 'dev/code/userinterface'],
    'cnc.ui.parts': ['dev/code/ui_left', 'dev/code/ui_right'],
    'cnc.utility': ['dev/code/idletimer', 'dev/code/millposition', 'dev/code/milltimer', 'dev/code/vector2'],
    'cnc.utility.wrapper': ['dev/code/colorwrapper', 'dev/code/commandparameterswrapper', 'dev/code/commandwrapper', 'dev/code/logentry', 'dev/code/programwrapper', 'dev/code/settingswrapper', 'dev/code/vector2wrapper'],
    'cnc.workspace': ['dev/code/mill'],
    'cnc.workspace.commands': ['dev/code/commandcoordinates', 'dev/code/commandtable', 'dev/code/millcommand'],
    'cnc.workspace.commands.g': ['dev/code/circle', 'dev/code/g00', 'dev/code/g01', 'dev/code/g02', 'dev/code/g03', 'dev/code/g28', 'dev/code/line'],
    'cnc.workspace.commands.m': ['dev/code/m00', 'dev/code/m02', 'dev/code/m03', 'dev/code/m04', 'dev/code/m05', 'dev/code/m08', 'dev/code/m09', 'dev/code/m13', 'dev/code/m14']
  },
};
