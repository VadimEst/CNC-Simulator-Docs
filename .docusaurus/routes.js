
import React from 'react';
import ComponentCreator from '@docusaurus/ComponentCreator';

export default [
  
{
  path: '/',
  component: ComponentCreator('/'),
  exact: true,
  
},
{
  path: '/docs/:route',
  component: ComponentCreator('/docs/:route'),
  
  routes: [
{
  path: '/docs/dev/architecture',
  component: ComponentCreator('/docs/dev/architecture'),
  exact: true,
  
},
{
  path: '/docs/dev/code/colorwrapper',
  component: ComponentCreator('/docs/dev/code/colorwrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/code/commandcoordinates',
  component: ComponentCreator('/docs/dev/code/commandcoordinates'),
  exact: true,
  
},
{
  path: '/docs/dev/code/commandparameterswrapper',
  component: ComponentCreator('/docs/dev/code/commandparameterswrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/code/commandtable',
  component: ComponentCreator('/docs/dev/code/commandtable'),
  exact: true,
  
},
{
  path: '/docs/dev/code/commandwrapper',
  component: ComponentCreator('/docs/dev/code/commandwrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/code/g00',
  component: ComponentCreator('/docs/dev/code/g00'),
  exact: true,
  
},
{
  path: '/docs/dev/code/g01',
  component: ComponentCreator('/docs/dev/code/g01'),
  exact: true,
  
},
{
  path: '/docs/dev/code/g02',
  component: ComponentCreator('/docs/dev/code/g02'),
  exact: true,
  
},
{
  path: '/docs/dev/code/g03',
  component: ComponentCreator('/docs/dev/code/g03'),
  exact: true,
  
},
{
  path: '/docs/dev/code/g28',
  component: ComponentCreator('/docs/dev/code/g28'),
  exact: true,
  
},
{
  path: '/docs/dev/code/idletimer',
  component: ComponentCreator('/docs/dev/code/idletimer'),
  exact: true,
  
},
{
  path: '/docs/dev/code/logentry',
  component: ComponentCreator('/docs/dev/code/logentry'),
  exact: true,
  
},
{
  path: '/docs/dev/code/logger',
  component: ComponentCreator('/docs/dev/code/logger'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m00',
  component: ComponentCreator('/docs/dev/code/m00'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m02',
  component: ComponentCreator('/docs/dev/code/m02'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m03',
  component: ComponentCreator('/docs/dev/code/m03'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m04',
  component: ComponentCreator('/docs/dev/code/m04'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m05',
  component: ComponentCreator('/docs/dev/code/m05'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m08',
  component: ComponentCreator('/docs/dev/code/m08'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m09',
  component: ComponentCreator('/docs/dev/code/m09'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m13',
  component: ComponentCreator('/docs/dev/code/m13'),
  exact: true,
  
},
{
  path: '/docs/dev/code/m14',
  component: ComponentCreator('/docs/dev/code/m14'),
  exact: true,
  
},
{
  path: '/docs/dev/code/mill',
  component: ComponentCreator('/docs/dev/code/mill'),
  exact: true,
  
},
{
  path: '/docs/dev/code/millcommand',
  component: ComponentCreator('/docs/dev/code/millcommand'),
  exact: true,
  
},
{
  path: '/docs/dev/code/milloutofboundsexception',
  component: ComponentCreator('/docs/dev/code/milloutofboundsexception'),
  exact: true,
  
},
{
  path: '/docs/dev/code/millposition',
  component: ComponentCreator('/docs/dev/code/millposition'),
  exact: true,
  
},
{
  path: '/docs/dev/code/milltimer',
  component: ComponentCreator('/docs/dev/code/milltimer'),
  exact: true,
  
},
{
  path: '/docs/dev/code/programparser',
  component: ComponentCreator('/docs/dev/code/programparser'),
  exact: true,
  
},
{
  path: '/docs/dev/code/programwrapper',
  component: ComponentCreator('/docs/dev/code/programwrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/code/settings',
  component: ComponentCreator('/docs/dev/code/settings'),
  exact: true,
  
},
{
  path: '/docs/dev/code/settingsparser',
  component: ComponentCreator('/docs/dev/code/settingsparser'),
  exact: true,
  
},
{
  path: '/docs/dev/code/settingspopover',
  component: ComponentCreator('/docs/dev/code/settingspopover'),
  exact: true,
  
},
{
  path: '/docs/dev/code/settingswrapper',
  component: ComponentCreator('/docs/dev/code/settingswrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/code/terminalreader',
  component: ComponentCreator('/docs/dev/code/terminalreader'),
  exact: true,
  
},
{
  path: '/docs/dev/code/tool',
  component: ComponentCreator('/docs/dev/code/tool'),
  exact: true,
  
},
{
  path: '/docs/dev/code/ui_left',
  component: ComponentCreator('/docs/dev/code/ui_left'),
  exact: true,
  
},
{
  path: '/docs/dev/code/ui_right',
  component: ComponentCreator('/docs/dev/code/ui_right'),
  exact: true,
  
},
{
  path: '/docs/dev/code/userinterface',
  component: ComponentCreator('/docs/dev/code/userinterface'),
  exact: true,
  
},
{
  path: '/docs/dev/code/vector2',
  component: ComponentCreator('/docs/dev/code/vector2'),
  exact: true,
  
},
{
  path: '/docs/dev/code/vector2wrapper',
  component: ComponentCreator('/docs/dev/code/vector2wrapper'),
  exact: true,
  
},
{
  path: '/docs/dev/contribute',
  component: ComponentCreator('/docs/dev/contribute'),
  exact: true,
  
},
{
  path: '/docs/dev/getting-started',
  component: ComponentCreator('/docs/dev/getting-started'),
  exact: true,
  
},
{
  path: '/docs/dev/libraries',
  component: ComponentCreator('/docs/dev/libraries'),
  exact: true,
  
},
{
  path: '/docs/dev/overview',
  component: ComponentCreator('/docs/dev/overview'),
  exact: true,
  
},
{
  path: '/docs/dev/scoping',
  component: ComponentCreator('/docs/dev/scoping'),
  exact: true,
  
},
{
  path: '/docs/dev/ui-concept',
  component: ComponentCreator('/docs/dev/ui-concept'),
  exact: true,
  
},
{
  path: '/docs/user/commands',
  component: ComponentCreator('/docs/user/commands'),
  exact: true,
  
},
{
  path: '/docs/user/installation',
  component: ComponentCreator('/docs/user/installation'),
  exact: true,
  
},
{
  path: '/docs/user/overview',
  component: ComponentCreator('/docs/user/overview'),
  exact: true,
  
}],
},
  
  {
    path: '*',
    component: ComponentCreator('*')
  }
];
